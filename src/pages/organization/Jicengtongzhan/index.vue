  ...
  <div>
    ...
    <div class="table-row">
      <el-table ref="multipleTable" :row-key="getRowKey" :data="list" tooltip-effect="dark" style="width: 100%"
        ...
        <el-table-column prop="party" label="党派类型" width="180">
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async getPageList() {
      //解构出参数
      const { department, page, limit } = this;
      axios
        .get(
          `http://localhost:9109/Organization/RenWu/Department/${department}/${page}/${limit}`,
        )
        .then((res)=>{
    ...
    //只输入了姓名
     searchSome() {
      ...
      axios
      .get(
        `http://localhost:9109/Organization/RenWu/Department/${department}/${this.searchMsg.name}/${page}/${limit}`
      )
      .then((res)=>{
        this.total = res.data.data.total
        this.list = res.data.data.info
